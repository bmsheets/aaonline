...
def array_sum_with_index(arr)
	...
	arr.map.with_index {|elem, idx| elem * idx}.reduce(:+)
end
...
def ordered_vowel_word?(word)
	vowels = ["a", "e", "i", "o", "u"]
	vowels_in_word = word.chars.select {|char| vowels.include?(char)}
...
def products_except_me(numbers)
	numbers.map {|num| array_product(numbers.reject {|n| n == num})}
end
